...
## 🌱 Sistema Inteligente del Invernadero Espacial
...
El operador debe registrar el porcentaje de humedad de **8 cultivos**.

El sistema debe:

- 🌱 Registrar la humedad.
- 📋 Mostrar todos los cultivos.
- 📈 Calcular el promedio.
- 🥇 Encontrar la mayor humedad.
- 🥈 Encontrar la menor humedad.
- ⚠️ Contar cuántos cultivos necesitan riego (humedad menor al 60%).
...
int main(){
    ...
    printf("===== INVERNADERO ESPACIAL =====\n\n");

    for(int i=0;i<CULTIVOS;i++){

        printf("Cultivo %d (%%): ",i+1);
        scanf("%f",&humedad[i]);

        suma+=humedad[i];

        if(humedad[i]>humedad[mayor])
            mayor=i;

        if(humedad[i]<humedad[menor])
            menor=i;
    ...
    printf("Mayor humedad : Cultivo %d (%.1f%%)\n",
           mayor+1,humedad[mayor]);

    printf("Menor humedad : Cultivo %d (%.1f%%)\n",
           menor+1,humedad[menor]);

    printf("Cultivos que necesitan riego: %d\n",riego);
